...
class easy_install(Command):
    ...
    def finalize_options(self):
        ...
        self._expand(
            "install_dir", "script_dir", "build_directory", "site_dirs",
        )
    ...
    def install_egg(self, egg_path, tmpdir):
        ...
        if not samefile(egg_path, destination):
            if os.path.isdir(destination) and not os.path.islink(destination):
                ...
                self.execute(
                    os.unlink, (destination,), "Removing " + destination,
                )
            try:
                ...
                update_dist_caches(
                    destination, fix_zipimporter_caches=new_dist_is_zipped,
                )
            except Exception:
    ...
    def update_pth(self, dist):
        ...
        if not self.multi_version:
            if dist.location in self.pth_file.paths:
                log.info(
                    "%s is already the active version in easy-install.pth", dist,
                )
            else:
    ...
    def byte_compile(self, to_compile):
        ...
        try:
            ...
            if self.optimize:
                byte_compile(
                    to_compile, optimize=self.optimize, force=1, dry_run=self.dry_run,
                )
        finally:
    ...
    DEFAULT_SCHEME = dict(
        install_dir="$base/Lib/site-packages", script_dir="$base/Scripts",
    )
